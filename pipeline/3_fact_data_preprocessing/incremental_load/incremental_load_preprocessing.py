...
df = spark.read.options(header=True, inferSchema=True).csv(f"{processed_path}/*.csv").withColumn("read_timestamp", F.current_timestamp())
...
df.write\
 ...
 .option("delta.enableChangeDataFeed", "true") \
 .mode("overwrite") \
 .saveAsTable(f"masterdata.{bronze_schema}.staging_{data_source}")
...
df_orders = spark.sql(f"SELECT * FROM masterdata.{bronze_schema}.staging_{data_source};")
df_orders.show(2)
...
df_joined.write\
 ...
 .option("delta.enableChangeDataFeed", "true") \
 .mode("overwrite") \
 .saveAsTable(f"masterdata.{silver_schema}.staging_{data_source}")
...
df_gold = spark.sql(f"SELECT order_id, order_placement_date as date, customer_id as customer_code, product_code, product_id, order_qty as sold_quantity FROM masterdata.{silver_schema}.staging_{data_source};")
...
df_child =  spark.sql(f"SELECT order_placement_date as date FROM masterdata.{silver_schema}.staging_{data_source}")
...
monthly_table = spark.sql(f"""
    ...
    FROM masterdata.{gold_schema}.fact_orders sbf
